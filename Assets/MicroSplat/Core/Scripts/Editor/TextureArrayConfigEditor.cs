...
namespace JBooth.MicroSplat
{
   ...
   public class TextureArrayConfigEditor : Editor
   {
      ...
      public override void OnInspectorGUI()
      {
         ...
         {
            cfg.packingMode = TextureArrayConfig.PackingMode.Fastest;
            cfg.pbrWorkflow = TextureArrayConfig.PBRWorkflow.Metallic;
            cfg.sourceTextureSize = TextureArrayConfig.SourceTextureSize.Unchanged;
         }

         cfg.diffuseIsLinear = EditorGUILayout.Toggle (CDiffuseIsLinear, cfg.diffuseIsLinear);

         if (cfg.IsScatter () || cfg.IsDecal())
         {
         ...
         bool isCluster = false)
      {
         ...
         if (cfg.emisMetalArray && !cfg.IsScatter () && !cfg.IsDecal())
         {
         ...
         for (int i = 0; i < src.Count; ++i)
         {
            try
            {
               ...
               if (cfg.emisMetalArray && !cfg.IsScatter () && !cfg.IsDecal () && emisTex != null && settings.emissiveSettings.compression != TextureArrayConfig.Compression.Uncompressed)
               {
         ...
         if (cfg.IsDecal() && splatArray != null)
         {
